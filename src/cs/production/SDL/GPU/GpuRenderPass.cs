...
/// <remarks>
///     <para>
///         <see cref="GpuRenderPass" /> instances are pooled and must not be used or referenced after calling
///         <see cref="GpuRenderPass.End" />. To get a <see cref="GpuRenderPass" /> instance call
///         <see cref="GpuCommandBuffer.BeginRenderPass" />.
///     </para>
/// </remarks>
[PublicAPI]
...
#pragma warning restore SA1401
    ...
    internal GpuRenderPass(GpuDevice device)
    {
        Device = device;
        CommandBuffer = null!;
        Handle = null;
    ...
    public void BindPipeline(GpuGraphicsPipeline? pipeline)
    {
    ...
    public void BindVertexBuffer(GpuDataBuffer? vertexBuffer, int offset = 0)
    {
    ...
    public void BindIndexBuffer(GpuDataBuffer? buffer, int offset = 0, int indexElementSize = 2)
    {
    ...
    ///     <paramref name="texture" /> usage flags are not <see cref="GpuTextureUsages.Sampler" />.
    /// </exception>
    public void BindFragmentSampler(GpuTexture? texture, GpuSampler? sampler)
    {
        ArgumentNullException.ThrowIfNull(texture);
        ArgumentNullException.ThrowIfNull(sampler);

        if ((texture.Usages & GpuTextureUsages.Sampler) == 0)
        {
    ...
    /// <summary>
    ///     Sets the <see cref="GpuViewport" /> of the render pass.
    /// </summary>
    ...
    public void SetViewport(in GpuViewport viewport)
    {
        ...
        fixed (GpuViewport* pointer = &viewport)
        {
